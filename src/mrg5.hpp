...
namespace trng {
  ...
  public:
    ...
    // Equality comparable concept
    friend bool operator==(const mrg5 &, const mrg5 &);
    friend bool operator!=(const mrg5 &, const mrg5 &);

    // Streamable concept
    template<typename char_t, typename traits_t>
    friend std::basic_ostream<char_t, traits_t> &
    operator<<(std::basic_ostream<char_t, traits_t> &out, const mrg5 &R) {
      std::ios_base::fmtflags flags(out.flags());
      out.flags(std::ios_base::dec | std::ios_base::fixed |
                std::ios_base::left);
      out << '[' << mrg5::name() << ' ' << R.P << ' ' << R.S << ']';
      out.flags(flags);
      return out;
    }
    ...
    template<typename char_t, typename traits_t>
    friend std::basic_istream<char_t, traits_t> &
    operator>>(std::basic_istream<char_t, traits_t> &in, mrg5 &R) {
      ...
      in >> utility::delim('[')
         >> utility::delim(mrg5::name()) >> utility::delim(' ')
         >> P_new >> utility::delim(' ')
         >> S_new >> utility::delim(']');
      if (in) {
        R.P=P_new;
...
//     double uniformcc(double, double) const;
  ...
  inline mrg5::result_type mrg5::operator()() const {
    step();
    return S.r1;
  }
  
  inline long mrg5::operator()(long x) const {
    return static_cast<long>(utility::uniformco(*this)*x);
  }

//   inline bool mrg5::boolean() const {
//     step();
//     return S.r1<=modulus/2;
//   }

//   inline bool mrg5::boolean(double p) const {
//     step();
//     return S.r1<modulus*p;
//   }
  
//   inline double mrg5::uniformco() const {
//     step();
//     return static_cast<double>(S.r1)/static_cast<double>(modulus);
//   }

//   inline double mrg5::uniformco(double a, double b) const {
//     return uniformco()*(b-a)+a;
//   }
  
//   inline double mrg5::uniformoc() const {
//     step();
//     return (static_cast<double>(S.r1)+1.0)/static_cast<double>(modulus);
//   }

//   inline double mrg5::uniformoc(double a, double b) const {
//     return uniformoc()*(b-a)+a;
//   }
  
//   inline double mrg5::uniformoo() const {
//     step();
//     return (static_cast<double>(S.r1)+1.0)/(static_cast<double>(modulus)+1.0);
//   }

//   inline double mrg5::uniformoo(double a, double b) const {
//     return uniformoo()*(b-a)+a;
//   }

//   inline double mrg5::uniformcc() const {
//     step();
//     return static_cast<double>(S.r1)/(static_cast<double>(modulus)-1.0);
//   }

//   inline double mrg5::uniformcc(double a, double b) const {
